...
from pape.timers import CudaTimer


class ImageClassificationModel(lightning.LightningModule):
    ...
    def training_step(self, batch: ImageClassificationBatch, batch_idx: int):
    ...
    def validation_step(self, batch: ImageClassificationBatch, batch_idx: int):
        batch_size = batch.images.size(0)
    ...
    def test_step(self, batch: ImageClassificationBatch, batch_idx: int):
        batch_size = batch.images.size(0)
        logits = self.classifier(batch)
        loss = F.cross_entropy(logits, batch.labels)
