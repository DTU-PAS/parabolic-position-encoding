...
from pape.nn.event_classifier import EventClassifier


class EventClassificationModel(lightning.LightningModule):
    ...
    def training_step(self, batch: EventsClassificationBatch, batch_idx: int):
        batch_size = batch.tokens.x.size(0)
        logits = self.classifier(batch)
        loss = F.cross_entropy(logits, batch.labels, label_smoothing=self.label_smoothing)
        self.log("train/loss", loss, on_step=True, on_epoch=True, prog_bar=True, batch_size=batch_size)
        return loss

    def validation_step(self, batch: EventsClassificationBatch, batch_idx: int):
        batch_size = batch.tokens.x.size(0)
        logits = self.classifier(batch)
        loss = F.cross_entropy(logits, batch.labels)
        self.log("val/loss", loss, batch_size=batch_size)

        sparse_labels = batch.labels.argmax(dim=-1)
        preds = logits.argmax(dim=-1)
        self.val_metrics.update(preds, sparse_labels)
    ...
    def test_step(self, batch: EventsClassificationBatch, batch_idx: int):
        batch_size = batch.tokens.x.size(0)
        logits = self.classifier(batch)
        loss = F.cross_entropy(logits, batch.labels)
        ...
        sparse_labels = batch.labels.argmax(dim=-1)
        preds = logits.argmax(dim=-1)
        self.test_metrics.update(preds, sparse_labels)
